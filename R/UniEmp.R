...
#' @param dist  The function for the plotting position formula (Gringorten or Weibull).
#' 
...
#' P<-UniEmp(X,dist = "Gringorten") 
#' barplot(t(P))
#' plot(1/P,X)

UniEmp<-function (X, dist = c("Weibull","Gringorten"))
{
  
  if (dist!= "Weibull" & dist != "Gringorten") 
  {
    stop ("Please select either Weibull or Gringorten 
          Other distribution forms will be updated soon.")
  }
